...
/obj/item/integrated_circuit/input/microphone
	...
	extended_desc = "This will automatically translate most languages it hears to English.  \
	The first activation pin is always pulsed when the circuit hears someone talk, while the second one \
	is only triggered if it hears someone speaking a language other than English."
	icon_state = "recorder"
	complexity = 8
	inputs = list()
	outputs = list(
...
/obj/item/integrated_circuit/input/microphone/hear_talk(mob/living/M, msg, var/verb="says", datum/language/speaking=null)
	...
	if(M && msg)
		if(speaking)
			...
			if(!istype(speaking, /datum/language/english))
				translated = TRUE
