...
/datum/event/carp_migration/announce()
	...
	if(severity == EVENT_LEVEL_MAJOR && deploy_drones)
		announcement = "Massive migration of unknown biological entities has been detected near [station_name()], please stand-by. The NDV Styx has dispatched combat drones to assist."
		soundfile = 'sound/AI/massivespacecarp.ogg'
	else
		announcement = "Unknown biological [length(spawned_carp) == 1 ? "entity has" : "entities have"] been detected near [station_name()], please stand-by.[severity == EVENT_LEVEL_MODERATE ? " The NDV Styx has dispatched combat drones to assist." : ""]"
