...
/datum/citizenship/empireofqueens
	name = CITIZENSHIP_EMPIRE_OF_QUEENS
	description = "The Empire Of Queens remains the second most powerful collective to currently hold power, while each hive on its own is weak, together the Empire stands as an equal to the Xal, \
	facing heavy discrimination in Human space, though with their homeworlds' extremely high phoron content, they are a wealthy and powerful ally to have. Vaurcae born within the Hive are the only eligible for citizenship."
...
/datum/citizenship/xal/get_objectives(mission_level, var/mob/living/carbon/human/H)
	...
	switch(mission_level)
		if(REPRESENTATIVE_MISSION_HIGH)
			...
			rep_objectives = pick("Question Non-Vaurcan employees about Vaurcan employees, looking for areas of improvement",
							"Protect and promote the public image of the Empire Of Queens to all Nanotrasen employees")

	return rep_objectives

/datum/outfit/job/representative/consular/empireofqueens
	name = "Empire Of Queens Consul Officer"
